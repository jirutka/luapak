...
local function build (proj_paths, entry_script, output_file, pkg_path, lua_lib, opts)
  ...
  for _, item in ipairs(proj_paths) do
    ...
    --local ok, err = 
    luarocks.build_and_install_rockspec(rockspec_file, proj_dir)
    -- if not ok then
...
return function (proj_paths, entry_script, output_file, rocks_dir, opts)
  ...
  if not lua_lib then
    ...
    if not lua_lib then
      errorf('Cannot find %s %s library. Please specify --lua-lib=PATH', lua_name, lua_ver or '')
    end
    log.debug('Using %s %s library: %s', lua_name, lua_ver or '', lua_lib)
